...
`
    ...
    for (const model of candidateModels) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(`[Groq Job Copilot Model ${model} failed]:`, lastError)
        }
      } catch (err: any) {
    ...
    if (!parsed || Object.keys(parsed).length === 0) {
      return { success: false, error: 'El copiloto de selección no pudo procesar la solicitud en este momento. Por favor inténtalo nuevamente.' }
    }
