...
import { formatPeriodo } from '@/lib/rcv-utils'
...
export function JournalClient({ 
  ...
  const formatDate = (dateStr: string) => {
    console.log("formatDate dateStr:", dateStr, typeof dateStr);
  ...
  const handlePeriodoChange = (value: string | null) => {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700" suppressHydrationWarning>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-muted/5 border border-border/50 p-6 rounded-[2rem] shadow-sm">
        ...
        <div className="w-full sm:w-64">
          <Select id="field_selectedperiodo_all" name="field_selectedperiodo_all" value={selectedPeriodo || 'all'} onValueChange={handlePeriodoChange}>
            ...
            <SelectContent className="rounded-2xl border-2 border-border shadow-2xl">
              <SelectItem value="all" className="text-xs font-bold uppercase tracking-wider rounded-xl">Todos (100 Recientes)</SelectItem>
              {availablePeriods.map((period) => (
                <SelectItem key={period} value={period} className="text-xs font-bold uppercase tracking-wider rounded-xl">
      ...
      {/* Listado de asientos o estado vacío */}
      {!entries || entries.length === 0 ? (
        <Card className="bg-card border-border shadow-2xl rounded-[2.5rem] overflow-hidden border-dashed border-2">
          <CardContent className="py-32 text-center" suppressHydrationWarning>
            ...
            <p className="text-muted-foreground font-bold uppercase text-xs tracking-widest italic">
              No se detectaron asientos contables en el período seleccionado.
            </p>
          </CardContent>
        </Card>
      ) : (
