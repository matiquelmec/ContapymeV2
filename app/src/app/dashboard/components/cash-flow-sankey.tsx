    ...
    <Card className="bg-card border-border shadow-2xl rounded-[2.5rem] overflow-hidden border-t-8 border-t-amber-500/10 flex flex-col h-full">
      <CardHeader className="bg-muted/5 border-b border-border p-5 sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-base sm:text-lg font-black text-foreground uppercase tracking-tight">Análisis de Flujo</CardTitle>
      ...
      <CardContent className="p-5 sm:p-8 flex-1 flex flex-col justify-center">
        <div className="flex flex-col gap-4 w-full max-w-md mx-auto relative">
          ...
          <div className="p-4 rounded-3xl border-2 bg-emerald-50/50 border-emerald-100 relative shadow-sm">
             ...
             <p className="text-xl sm:text-2xl font-black text-emerald-700 tracking-tighter">{fCLP(financials.totalSales)}</p>
             <div className="absolute left-1/2 -bottom-4 w-1 h-4 bg-border -translate-x-1/2" />
          </div>

          {/* Salidas Ramificadas */}
          <div className="flex gap-4 isolate">
            <div className="flex-1 p-3 sm:p-4 rounded-3xl border-2 bg-rose-50/50 border-rose-100 flex flex-col justify-center relative mt-4 shadow-sm">
              ...
              <p className="text-sm sm:text-base md:text-lg font-black text-rose-700 tracking-tighter">{fCLP(financials.totalPurchases)}</p>
            </div>
            
            <div className="flex-1 p-3 sm:p-4 rounded-3xl border-2 bg-orange-50/50 border-orange-100 flex flex-col justify-center relative mt-4 shadow-sm">
              ...
              <p className="text-sm sm:text-base md:text-lg font-black text-orange-700 tracking-tighter">{fCLP(financials.totalPayroll)}</p>
            </div>
          </div>

          {/* Margen Final */}
           <div className={`p-4 sm:p-5 rounded-3xl border-4 relative mt-4 flex items-center justify-between shadow-md transition-all ${financials.ebitda >= 0 ? 'bg-primary/5 border-primary/20' : 'bg-rose-50 border-rose-200'}`}>
             ...
             <div>
                ...
                <p className={`text-xl sm:text-2xl font-black tracking-tighter ${financials.ebitda >= 0 ? 'text-primary' : 'text-rose-700'}`}>
                   {fCLP(financials.ebitda)}
                </p>
             </div>
             <div className="text-right">
                <span className={`px-2 sm:px-3 py-1 rounded-full font-black text-[10px] sm:text-xs uppercase tracking-widest shadow-sm text-white ${financials.ebitda > 0 ? 'bg-primary' : financials.ebitda < 0 ? 'bg-rose-600' : 'bg-slate-400'}`}>
