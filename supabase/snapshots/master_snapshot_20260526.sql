-- CONTAPYMEPUQ MASTER SCHEMA SNAPSHOT
-- DATE: 2026-05-26
-- STATUS: PRODUCTION SINCRO TOTAL (v9.0)
...
CREATE TABLE public.centralized_account_config (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  tax_iva_debito_code text,
  tax_iva_debito_name text,
  tax_iva_credito_code text,
  tax_iva_credito_name text,
  tax_ppm_code text,
  tax_ppm_name text,
  tax_retentions_code text,
  tax_retentions_name text,
  tax_f29_payable_code text,
  tax_f29_payable_name text,
  tax_iva_remanente_code text,
  tax_iva_remanente_name text,
  asset_depreciation_expense_code text,
  asset_depreciation_expense_name text,
  asset_accumulated_depreciation_code text,
  asset_accumulated_depreciation_name text,
  expense_salary_code text,
  expense_salary_name text,
  expense_social_code text,
  expense_social_name text,
  liability_afp_code text,
  liability_afp_name text,
  liability_salud_code text,
  liability_salud_name text,
  liability_afc_code text,
  liability_afc_name text,
  liability_tax_code text,
  liability_tax_name text,
  liability_net_code text,
  liability_net_name text,
...
CREATE TABLE public.chart_of_accounts (
  ...
  CONSTRAINT fk_coa_parent_hierarchy FOREIGN KEY (organization_id, parent_codigo) REFERENCES public.chart_of_accounts(organization_id, codigo) ON DELETE RESTRICT
);
...
CREATE TABLE public.dte_caf_folios (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  ...
  environment text NOT NULL DEFAULT 'certification'::text CHECK (environment = ANY (ARRAY['certification'::text, 'production'::text])),
  caf_xml text NOT NULL,
  is_active boolean DEFAULT true,
  authorized_at date,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT dte_caf_folios_pkey PRIMARY KEY (id),
  CONSTRAINT dte_caf_folios_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id),
  CONSTRAINT dte_caf_folios_company_id_fkey FOREIGN KEY (company_id) REFERENCES public.dte_companies(id)
);
CREATE TABLE public.dte_companies (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  rut text NOT NULL,
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT dte_companies_pkey PRIMARY KEY (id),
  CONSTRAINT dte_companies_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id)
);
CREATE TABLE public.dte_issued (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL,
  company_id uuid NOT NULL,
  tipo_dte integer NOT NULL,
  ...
  tasa_iva numeric NOT NULL DEFAULT 19.00,
  ...
  updated_at timestamp with time zone DEFAULT now(),
  created_by uuid,
  CONSTRAINT dte_issued_pkey PRIMARY KEY (id),
  CONSTRAINT dte_issued_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id),
  CONSTRAINT dte_issued_company_id_fkey FOREIGN KEY (company_id) REFERENCES public.dte_companies(id),
  CONSTRAINT dte_issued_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id)
);
CREATE TABLE public.dte_items (
  ...
  CONSTRAINT dte_items_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id),
  CONSTRAINT dte_items_dte_id_fkey FOREIGN KEY (dte_id) REFERENCES public.dte_issued(id) ON DELETE CASCADE
);
...
CREATE TABLE public.fixed_assets (
  ...
  metodo_depreciacion text NOT NULL DEFAULT 'lineal'::text,
  condicion text NOT NULL DEFAULT 'activo'::text,
